...
def update_page(n_intervals):
        ...
        try:
            ...
            for plant in plant_misc:
                ...
                plant_lastwatered.append(conn.execute(
                    select([watering_table.c.timestamp]).where(watering_table.c.plant_id == plant[2]).limit(1).order_by(desc(watering_table.c.timestamp))
                ).fetchall())
    ...
    return [make_gauge("Temperature", temp_result[0][0], "° F", [0, 100], "#dc3545"), make_gauge("Relative Humidity", humidity_result[0][0], "%", [0, 100], "#17a2b8"), 
     px.line(sunlight_df, x='timestamp', y=sunlight_df.value, title='Sunlight', width=525, height=250).update_yaxes(autorange=False, range=[0,100]),
     px.line(plant1_df, x='timestamp', y=plant1_df.value, width=450, height=300).update_yaxes(autorange=False, range=[0,100]),
     px.line(plant2_df, x='timestamp', y=plant2_df.value, width=450, height=300).update_yaxes(autorange=False, range=[0,100]),
     px.line(plant3_df, x='timestamp', y=plant3_df.value, width=450, height=300).update_yaxes(autorange=False, range=[0,100]),
     px.line(plant4_df, x='timestamp', y=plant4_df.value, width=450, height=300).update_yaxes(autorange=False, range=[0,100]), # plant humidity graphs
