...
from sqlalchemy import Table, create_engine, desc, select, text
...
def update_page(n_intervals):
    ...
    error = 0
    humidity_result, temp_result, light_result, plant_graphs, plant_misc = (
    ...
    if error == 1 or None in [humidity_result, temp_result, light_result, plant_graphs, plant_misc]:
    ...
    return [make_gauge("Temperature", temp_result[0][0], "° F", [0, 100], "#dc3545"), make_gauge("Relative Humidity", humidity_result[0][0], "%", [0, 100], "#17a2b8"),
            px.line(sunlight_df, x='timestamp', y=sunlight_df.value,
                    title='Sunlight', width=525, height=250),
            px.line(plant1_df, x='timestamp',
                    y=plant1_df.value, width=450, height=300),
            px.line(plant2_df, x='timestamp',
                    y=plant2_df.value, width=450, height=300),
            px.line(plant3_df, x='timestamp',
                    y=plant3_df.value, width=450, height=300),
            px.line(plant4_df, x='timestamp', y=plant4_df.value,
                    width=450, height=300),  # plant humidity graphs
            # names
